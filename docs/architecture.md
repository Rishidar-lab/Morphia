...
**Scope of what "execution" means here:** a provider call is an LLM chat/completion invocation. A `RunStep`'s `output_data` holds the model's text response plus token/cost accounting; on its own it is not evidence that a scan actually ran. A plan step tagged `"tool": "httpx"` instead dispatches to a real `ToolAdapter` (`apps/worker/app/tools/`) — the step's `output_data` is that binary's actual `stdout`/`stderr`/`exit_code`/`argv`/version, captured under a hard timeout and output-size cap, with `"source": "tool"` distinguishing it from a model response (`"source": "model"`). httpx is the first tool wired this way; only it, not the generic ability to run arbitrary binaries. Both paths go through the identical claim-time `ScopeValidator` call — there is no separate, unchecked dispatch route for tool execution. Provenance is not yet formalized beyond that JSON marker: a dedicated DB column, API field, UI treatment, and disclosure-report distinction between tool- and model-sourced evidence remain unbuilt.
